  ...
  switch (action.type) {
    ...
    case ADD_NEW_DUCK_ID_TO_FEED:
      return {
        ...state,
        newDucksToAdd: [
          action.duckId, ...state.newDucksToAdd
        ]
      }
...
export default function usersLikes(state = initialState, action) {
  switch (action.type) {
    ...
    case REMOVE_LIKE:
      return Object.keys(state).filter((duckId) => action.duckID !== duckId).reduce((previous, current) => {
...
function repliesAndLastUpdated(state = initialDuckState, action){
  switch (action.type) {
    ...
    return {
      ...state,
      replies: duckReplies(state[action.duckId], action)
    }
